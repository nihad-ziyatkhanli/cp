            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    @foreach ($menu_data as $mi)
                        @if (isset($mi['url']))
                            <x-nav-link :href="$mi['url']" :active="$mi['active']">
                                {{ __($mi['title']) }}
                            </x-nav-link>
                        @else
                            <x-nav-dropdown class="dropdown relative" :active="$mi['active']">
                                ...
                                <ul class="dropdown-menu min-w-full absolute hidden bg-white text-base z-50 float-left py-2 list-none text-left rounded-lg shadow-lg mt-1 hidden m-0 bg-clip-padding border-none"
                                    ...
                                    @foreach ($mi['children'] as $child)
                                        <li>
                                            <a  class="{{ $child['active'] }} dropdown-item text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent text-gray-700 hover:bg-gray-100"
